...
export default ({id}) => {
    ...
    return (
        <div style={open ? {...s.serviceWrapper, ...s.serviceOpen} : {...s.serviceWrapper}}>
            <div style={s.header}>
                <svg id={`${id}-l`} style={s.leftConnection} height="12" width="12"><circle cx="6" cy="6" r="5" stroke="black" stroke-width="1" fill={c.steel}/></svg> 
                <img style={s.img}/>
                <p onClick={() => setOpen(!open)} style={s.text}>Electrical Connection Sensor</p>
                ...
                <svg id={`${id}-r`} style={s.rightConnection} height="12" width="12"><circle cx="6" cy="6" r="5" stroke="black" stroke-width="1" fill={c.steel}/></svg> 
            </div>
            {open && 

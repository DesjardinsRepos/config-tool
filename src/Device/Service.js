...
const Connection = ({ser, direction}) => {
    ...
    const inputString = direction == "left" ? "consumer" : "producer"
    const outputString = direction == "left" ? "producer" : "consumer"
    ...
    return (
        <div style={{display: "flex"}} 
            ...
            onMouseUp={()=> {
                if(selected?.currentlyLineDrawing) { // create connection
                    ...
                    const [connection, connectionHash] = require("../general.js").createCanvasConnection(
                        {
                            id: ser.id,
                            direction: direction
                        },
        ...
        >
            ...
            <div style={{position: "relative", display: "flex"}}>
                ...
                {(ser.serviceDirection === inputString) && <svg style={{margin: "auto 11px auto 11px"}} height="12" width="13.44">
                <polygon points="2.8,1 11.8,5.75 2.8,11" fill={ser.id === selected.id ? c.darkSteel : c.steel} stroke="black" strokeWidth="1" />
                </svg>}

                {(ser.serviceDirection === outputString) && <svg style={{margin: "auto 11px auto 11px"}} height="12" width="13.44">

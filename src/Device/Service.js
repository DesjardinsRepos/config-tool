...
export default ({ser}) => {
    ...
    return (
        <div style={open ? {...s.serviceWrapper, ...s.serviceOpen} : {...s.serviceWrapper}}>
            <div style={s.header}>
                <svg id={`${ser.parentId}.${ser.id}-l`} style={s.leftConnection} height="12" width="12"><circle cx="6" cy="6" r="5" stroke="black" strokeWidth="1" fill={c.steel}/></svg> 
                <img style={s.img}/>
                ...
                <svg id={`${ser.parentId}.${ser.id}-r`} style={s.rightConnection} height="12" width="12"><circle cx="6" cy="6" r="5" stroke="black" strokeWidth="1" fill={c.steel}/></svg> 
            </div>
            {open && 

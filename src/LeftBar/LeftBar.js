    ...
    return (
        <div style={s.wrapperToFixAutoResize}>
            <div style={s.barWrapper}>
                {apiLoading && <Skeleton count={3} baseColor="#CCDCEC" height="40px" style={{marginBottom: "5px"}}/>}
                    
                {!apiLoading && !apiError && Object.keys(devices).map((key, i) =>
                    <DeviceDropdown name={key} devices={devices[key]} key={i}/>)}

                {!apiLoading && apiError && 
                    <div style={{display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100px", paddingTop: "20px"}}> 
                        <p style={{margin: "auto", textAlign: "center"}}>Failed to request devices - {apiError}</p>
                        <button style={{width: "200px", margin: "20px auto auto auto", height: "40px", borderRadius: "20px", backgroundColor: c.lightBlue, border: `2px solid ${c.steel}`, cursor: "pointer"}} onClick={() => {
                            setApiLoaded(false)    
                            setApiError(false)
                            requestDevices()
                        }}>Reload</button>
